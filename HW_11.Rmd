...
AH: Need to create a sum by row and then divide yes by total to get rates.
...
```
AH: A stronger answer would demonatrate how these functions work as in:
```{r}
prism_dat <- load_data ("data/")
tcheck(prism_dat[[1]], 0) # for single data frame
tcheck_all(prism_dat, 0) # for a list of dataframes
```
**3) Write a loop to extract Cape Grim monthly temperature data (GHCN) for 1985-2019 using rnoaa.**
  
```{r}
data = data.frame()
for(year in 1985:1987){ #to limit my downloads!
